...
- 左電極電位（アノード）: $\phi_L = +50$ mV
- 右電極電位（カソード）: $\phi_R = -50$ mV
- バルク濃度: $c_0 = 1.0$ mol/L
- 計算領域: $L = 100$ nm
...
![Dual Electrode](results/dual_electrode.png?v=4)

*図1: EMI-BF₄イオン液体の両電極モデル（φ_L = +50 mV, φ_R = -50 mV）。(a) 電位プロファイル（全領域）、(b) イオン濃度分布（対数スケール）、(c) 左電極（アノード）近傍のEDL構造、(d) 右電極（カソード）近傍のEDL構造。(c), (d)では左軸に電位、右軸に濃度（線形スケール）を表示。Debye長（λ_D = 119 pm）およびイオン直径（EMI⁺: 760 pm、BF₄⁻: 460 pm）をマーカーで表示。*

</div>

<div align="center">

![Grid Points](results/dual_electrode_grid_points.png?v=4)
...
| バルク (x = L/2) | 0.0 | 1000.0 | 1.00 | 1000.0 | 1.00 |
| 右端 (x = L) | -50.0 | 7001.2 | 7.00 | 142.8 | 0.143 |

**物理的解釈**:

- 正電位の左電極近傍（アノード）: カチオン（EMI⁺）が排除され（c+/c₀ = 0.14）、アニオン（BF₄⁻）が蓄積（c-/c₀ = 7.0）
- 負電位の右電極近傍（カソード）: アニオン（BF₄⁻）が排除され（c-/c₀ = 0.14）、カチオン（EMI⁺）が蓄積（c+/c₀ = 7.0）
- バルク領域（x ≈ L/2）: 電気的中性（c+/c₀ = c-/c₀ = 1.0）、電位 ≈ 0 mV
- 両電極でEDLが対称的に形成される（電位差 = ±50 mV）
