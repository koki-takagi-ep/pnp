...
電圧印加から定常状態に達するまでの時間発展をアニメーションで可視化できる。

<div align="center">

![EDL Evolution](results/edl_evolution.gif)

*図: 電気二重層形成過程のアニメーション。左: 電位分布の時間発展（青: 過渡解、赤破線: 定常解析解）、右: イオン濃度分布（赤: カチオン、青: アニオン）。*
...
- バルク濃度: $c_0 = 0.1$ mol/L
- 時間刻み: $\Delta t = 1$ ns
- 終了時間: $t_{\text{final}} = 5$ µs

電圧印加直後から、表面近傍でイオンの再配置が始まり、拡散時間スケール（$\tau_D = \lambda_D^2/D \sim 1$ ns）でEDLが形成される様子が観察できる。
