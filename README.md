...
- 左電極電位（アノード）: $\phi_L = +100$ mV
- 右電極電位（カソード）: $\phi_R = 0$ mV
- バルク電位（自己無撞着に決定）: $\phi_{\text{bulk}} = +50$ mV
- バルク濃度: $c_0 = 1.0$ mol/L
- 計算領域: $L = 100$ nm
...
![Dual Electrode](results/dual_electrode.png?v=5)

*図1: EMI-BF₄イオン液体の両電極モデル（φ_L = +100 mV, φ_R = 0 mV, φ_bulk = 50 mV）。(a) 電位プロファイル（全領域）、(b) イオン濃度分布（対数スケール）、(c) 左電極（アノード）近傍のEDL構造、(d) 右電極（カソード）近傍のEDL構造。(c), (d)では左軸に電位、右軸に濃度（線形スケール）を表示。Debye長（λ_D = 119 pm）およびイオン直径（EMI⁺: 760 pm、BF₄⁻: 460 pm）をマーカーで表示。*

</div>

<div align="center">

![Grid Points](results/dual_electrode_grid_points.png?v=5)
...
|:-----|----------:|------------:|------:|------------:|------:|
| 左端 (x = 0) | +100.0 | 142.8 | 0.143 | 7001 | 7.00 |
| バルク (x = L/2) | +50.0 | 1000.0 | 1.00 | 1000.0 | 1.00 |
| 右端 (x = L) | 0.0 | 7001 | 7.00 | 142.8 | 0.143 |

**物理的解釈**:

- 左電極（φ = +100 mV、バルク基準で+50 mV）: カチオン（EMI⁺）が排除され（c+/c₀ = 0.14）、アニオン（BF₄⁻）が蓄積（c-/c₀ = 7.0）
- 右電極（φ = 0 mV、バルク基準で-50 mV）: アニオン（BF₄⁻）が排除され（c-/c₀ = 0.14）、カチオン（EMI⁺）が蓄積（c+/c₀ = 7.0）
- バルク領域（x ≈ L/2）: 電気的中性（c+/c₀ = c-/c₀ = 1.0）、電位 = 50 mV
- 両電極でEDLが対称的に形成される（バルク基準での実効電位差 = ±50 mV）
- 濃度比 exp(±50 mV / 25.7 mV) ≈ 7.0 は Boltzmann 分布と一致
...
開放系では右端の境界条件が $\phi = 0$, $c = c_0$ に固定されるため、バルク電位は自動的に 0 V となる。しかし閉鎖系では：

1. 電位境界条件は両電極で固定（Dirichlet）: $\phi(0) = \phi_L$, $\phi(L) = \phi_R$
2. 濃度境界条件はゼロフラックス（Neumann）: $J(0) = J(L) = 0$
3. 系全体で総電荷がゼロ: $\int_0^L \rho \, dx = 0$

ユーザーが例えば $\phi_L = +100$ mV, $\phi_R = 0$ mV を指定した場合、単純にこの境界条件で Poisson-Boltzmann 方程式を解くと、左電極でのみ EDL が形成され、右電極ではバルク条件（$\phi = 0$）となるため、**総電荷がゼロにならない**。

**解決策**:

本ソルバーでは、閉鎖系において以下のアルゴリズムで電荷中性を満たすバルク電位を決定する：

1. **電位差の保存**: $\Delta\phi = \phi_L - \phi_R$ は常に保存
2. **オフセットの導入**: 実効的な境界条件を $\phi_L^{\text{eff}} = \phi_L + \delta$, $\phi_R^{\text{eff}} = \phi_R + \delta$ とする
3. **電荷中性条件**: オフセット $\delta$ を調整して $Q = \int \rho \, dx = 0$ を満たす
4. **初期推定**: 対称な場合、$\delta = -(\phi_L + \phi_R)/2$ でバルク電位が 0 になる

**例**:

```bash
# ユーザー指定: +100 mV / 0 mV
./build/pnp_solver --phi0 100 --phi-right 0 --closed-system

# 結果:
# - 電荷中性を満たすオフセット: δ = -50 mV
# - 実効境界条件: +50 mV / -50 mV
# - バルク電位（中央）: ≈ 0 mV = (100 + 0)/2 - 50
```
...
- **Sugioka, Phys. Rev. E 86, 016318 (2012)**: "Ion-conserving Poisson-Boltzmann theory" - ICPB理論の原論文。イオン保存条件 $\int (c_+ + c_-) dx = 2c_0 L$ を課すことで、GC解とは異なる物理的に正しい解が得られることを示した
...
| 条件 | バルク電位の決定 |
|------|-----------------|
| 対称な1:1電解質（\|z₊\| = \|z₋\| = 1） | $\phi_{\text{bulk}} = (\phi_L + \phi_R)/2$ |
| 電荷中性条件 $\int \rho \, dx = 0$ | 数学的に一意に決定 |
| 境界条件 + 電荷中性 | 自己無撞着解 |

例えば、$\phi_L = +100$ mV, $\phi_R = 0$ mV の場合、電荷中性条件から $\phi_{\text{bulk}} = 50$ mV が**厳密に**導かれる。これは仮定ではなく、Poisson-Boltzmann方程式と電荷中性条件から導かれる数学的帰結である。
...
**重要**: 本ソルバーの閉鎖系モードは**平衡状態**（電流ゼロ、ゼロフラックス境界条件）を計算する。この条件下では、電荷中性から決定されるバルク電位 $\phi_{\text{bulk}} = (\phi_L + \phi_R)/2$ は物理的に正確である。参照電極を明示的にモデルに含める必要はない。
...
1. **連続体仮定の破綻**: PB理論はイオンを点電荷として扱う連続体近似に基づく。λ_D がイオンサイズより小さい場合、この仮定は物理的に妥当でない
