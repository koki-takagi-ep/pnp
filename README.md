...
継続法（continuation method）を用いて、表面電位を0から目標値まで徐々に増加させながら各電位での定常解を計算し、EDL形成過程をアニメーションで可視化する。

<div align="center">

![EDL Evolution](results/edl_evolution.gif)

*図: EDL形成過程のアニメーション。左: 電位分布（青: 数値解、赤破線: Gouy-Chapman解析解）、右: イオン濃度分布（赤: カチオン、青: アニオン）。表面電位を0→100 mVまで徐々に増加させ、各電位での定常状態を表示。*

</div>

**シミュレーション条件**:
- 表面電位: 0 → 100 mV（50ステップ）
- バルク濃度: $c_0 = 0.01$ mol/L
- 各ステップで定常状態Newton-Raphson解を計算

**時定数の推定**:
- Debye長: $\lambda_D = 1.19$ nm
- RC時定数: $\tau_{RC} = \lambda_D \cdot L / D \approx 595$ ns

時間軸は指数関数的充電プロファイル $V(t) = V_0(1 - e^{-t/\tau_{RC}})$ に基づいて推定。電位の増加に伴い、表面近傍でカチオンが排除されアニオンが蓄積し、電気二重層が発達する様子が観察できる。
