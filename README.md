...
陰的 Scharfetter-Gummel スキームによる真の時間発展計算で、EDL形成過程をシミュレーションする。

<div align="center">

![EDL Evolution](results/edl_evolution.gif)

*図: EDL時間発展のアニメーション（0〜1 µs）。左: 電位分布（青: 数値解、赤破線: Gouy-Chapman解析解）、右: イオン濃度分布（赤: カチオン c+、青: アニオン c-）。初期状態（一様濃度）から定常状態への緩和過程を表示。*

</div>

**シミュレーション条件**:
- 表面電位: 100 mV（Dirichlet BC）
- バルク濃度: $c_0 = 1.0$ mol/L
- 時間刻み: $\Delta t = 0.1$ ns
- 計算時間: 1 µs（10,000ステップ）
...
**定常状態の濃度**:
- 表面での c+/c₀ ≈ 0.02（カチオン排除）
- 表面での c-/c₀ ≈ 46（アニオン蓄積）

電位印加直後から急速にイオン再分布が始まり、約200 ns で定常状態に収束する。表面近傍でカチオンが排除されアニオンが蓄積し、電気二重層が形成される様子が観察できる。
