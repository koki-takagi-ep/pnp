...
*図1: EMI-BF₄イオン液体の両電極モデル。(a) 電位プロファイル（全領域）、(b) イオン濃度分布（対数スケール）、(c) 左電極（アノード）近傍のEDL構造、(d) 右電極（カソード）近傍のEDL構造。(c), (d)では左軸に電位、右軸に濃度（線形スケール）を表示。Debye長（λ_D = 119 pm）およびイオン直径（EMI⁺: 760 pm、BF₄⁻: 460 pm）をマーカーで表示。*
