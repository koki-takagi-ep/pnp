...
電圧印加後の電気二重層形成過程を時間発展でシミュレーションする。

<div align="center">

![EDL Evolution](results/edl_evolution.gif)

*図: EDL形成の時間発展アニメーション。左: 電位分布（青: 過渡解、赤破線: 定常解析解）、右: イオン濃度分布（赤: カチオン、青: アニオン）。初期状態（均一濃度・線形電位）から定常状態へ向かう緩和過程を可視化。*

</div>

**シミュレーション条件**:
- 表面電位: $\phi_0 = 100$ mV（ステップ印加）
- バルク濃度: $c_0 = 0.01$ mol/L
- 時間刻み: $\Delta t = 0.001$ ns
- 終了時間: $t_{\text{final}} = 500$ ns

**時定数の推定**:
- Debye長: $\lambda_D = 1.19$ nm
- 拡散時定数: $\tau_D = \lambda_D^2/D \approx 1.4$ ns
- RC時定数: $\tau_{RC} = \lambda_D \cdot L / D \approx 60$ ns

電圧印加直後、イオンの再分布が始まり、拡散時定数（$\sim 1$ ns）程度でEDL内部の構造が形成され、RC時定数（$\sim 60$ ns）程度で全体が定常状態に収束する。
